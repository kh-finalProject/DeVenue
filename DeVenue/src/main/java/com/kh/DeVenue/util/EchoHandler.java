...
//        	sess.sendMessage(new TextMessage("에고 : " + message.getPayload()));
//        }
        //HttpSession에 담은 로그인 유저 객체 가져오기(servlet-context.xml에 인터셉터 설정을 해줘야 함)
        Member loginUser;
...
//            sessions.get(sessionId).sendMessage(new TextMessage(session.getId()+"|" + message.getPayload()));            
            ...
            sessions.get(sessionId).sendMessage(new TextMessage(loginUser.getmEmail()+"|" + message.getPayload()));
